  ...
  const handleWithdraw = async () => {
    ...
    try {
      const res = await postWithdraw({
        ...
        accountHolder: account.fullName || account.name || '',
        amount: amt,
          ...
          <div style={{ ...S.screenInner, alignItems: 'stretch' }}>
            <div style={{ textAlign: 'center', marginBottom: 12 }}>
              ...
              <p style={{ color: '#4ade80', fontSize: '0.85rem', fontWeight: 700, margin: 0 }}>
                Welcome, {account?.name?.split(' ')[0] || 'Customer'}
              </p>
      ...
      case 'balance':
        return (
          <div style={S.screenInner}>
            ...
            <p style={{ color: '#e2e8f0', fontSize: '0.9rem', fontWeight: 700, margin: '0 0 16px' }}>{account?.name}</p>
